...
def run_mtmfea(experiment_name, structure_shape, pop_size, max_evaluations, train_iters, num_cores, env_name):
    print()
    ...
    while True:
        ...
        ### TRAIN GENERATION
        tc_fidelity = 100
        if num_evaluations < max_evaluations / 3:
            tc_fidelity = round(1/3*tc)
        elif max_evaluations / 3 <= num_evaluations < max_evaluations *2 / 3:
            tc_fidelity = round(2/3 * tc)
        elif num_evaluations >= max_evaluations *2 / 3:
            tc_fidelity = tc
        ...
        for structure in structures:
            ...
            if structure.is_survivor:
                ...
                ppo_args = ((structure.body, structure.connections), tc_fidelity, (save_path_controller, structure.label), env_name)
