...
from layout_data.models.model import FCNSL, UnetSL
...
def main(hparams):
    ...
    model = UnetSL(hparams).cuda()
    model_path = '/example/lightning_logs/version_4/checkpoints/epoch=29-step=20249.ckpt'
    model.load_state_dict(torch.load(model_path)['state_dict'])
    model.prepare_data()
    data_loader = model.test_dataloader()
    mae_list, cmae_list, maxae_list, mtae_list = [], [], [], []
    ...
    sio.savemat('complex_batch_size_1_test.mat', {'mae': mae_list,
                             'cmae': cmae_list,
                             'maxae': maxae_list,
                             'mtae': mtae_list})
